...
let NFT_array = [];
let counter = 0; //Change to start index

const interval = setInterval(async () => {

  if (counter < 1500) { //Change to end index
    for (let i = 0; i < 50; i++) {
      try {
...
async function obtain_data(i) {
  https.get(`https://gateway.pinata.cloud/ipfs/QmRcEveDugLLwktAV2F7KdxBvxRDx3BgvLM97wGopehV9E/${i}`, async (resp) => {
...
function writeFile(data) {
  console.log(`WRITING NOW to daoturtle_data_${counter-1}.json!`)
  fs.writeFileSync(`./daoturtle_data_${counter-1}.json`, JSON.stringify(NFT_array)); //Switch this
}